...
NOTA: Cuando se utiliza en el telefono la aplicacion y se abre la ventana modal donde se pide que se ingrese el kilometraje y la foto del comprobante del mismo, si de inmediato se presiona el boton de "Tomar Foto" la aplicacion se traba y no deja hacer nada hasta que se le de atras en el telefono, Esto SOLO sucede cuando el selector de imagen esta puesto en "Tomar Foto" (marcado como azul) y se vuelve a presionar este boton.
...
- **Resultado Esperado (Happy Path):**
  - [x] **Kilometraje Recorrido:** Muestra la sumatoria total de kilómetros de viajes completados y cerrados (ej: `1.130 km`) con subtítulo indicando cantidad de viajes completados.
  - [x] **Rendimiento Promedio:** Muestra el cálculo de eficiencia en formato decimal `XX,XX km/gal` calculado como `total_km / total_galones`.
...
- **Resultado Esperado (Happy Path):**
  - [x] Muestra la proporción exacta de vehículos según su estado actual: `Disponible` (verde), `Asignado` (azul/info), `En Viaje` (amarillo/warning), `Mantenimiento` (rojo/danger) y `Desincorporado` (gris).
  - [x] Al pasar el cursor sobre cada segmento (hover), muestra la etiqueta del estado y la cantidad exacta de vehículos.
...
- **Resultado Esperado (Happy Path):**
  - [x] El gráfico se actualiza dinámicamente recalculando los kilómetros recorridos agrupados por mes cronológico.
  - [x] Cada barra o punto refleja fielmente la suma de distancias de los viajes con fecha de salida o llegada en el mes correspondiente.
...
- **Pasos:**
  ...
  NOTA: EN el grafico se muestra el top 5 de solicitantes de viajes, seria bueno mejorar la visibilidad colocando un color diferente a la barra que representa a cada uno.
- **Resultado Esperado (Happy Path):**
  - [x] Clasifica a las empresas o clientes que más viajes han completado (ej: *Consorcio Vial*, *Alimentos del Centro*, *Logística Nacional*) (El grafico claisifca a personas como tal, no empresas, ya que se muestra el nombre de las personas solicitantes, pero no el nombre de la empresa a la que pertenecen, y no es lo mismo, si el top 5 solicitantes fuese de una misma empresa, si el grafico clasificara a las EMPRESAS solo saldria una barra, pero si se clasifica a las personas como tal, aunque todo el top 5 sea de la misma empresa se deben colocar barras individuales para cada cual, esto es una observacion).
  - [x] En *Mes Actual*, filtra únicamente los viajes concluidos en el mes en curso.
...
- **Resultado Esperado (Happy Path):**
  - [x] Para conductores en viaje (ej: Jorge Gaitán en `RUN-808`), se visualiza el código del viaje (`TRIP-2026-0006`), el destino (`Sede Operativa Ibagué`), la placa asignada, el teléfono de contacto y el tiempo transcurrido en ruta calculado dinámicamente (ej: `3h 15m`).
  - [x] Para conductores disponibles (ej: Juan Pablo Montoya), se visualiza el badge verde `Disponible`.
  - [x] Si un conductor tiene la licencia vencida (ej: Ricardo Arjona), se destaca con badge rojo `Licencia Vencida`.
...
- **Resultado Esperado (Happy Path):**
  - [x] Lista de manera destacada a los conductores cuya licencia de conducción ya venció o se encuentra próxima a vencer en los próximos 30 días.
  - [x] Se muestra el nombre del conductor, número de documento, categoría (`C1`, `C2`, etc.), fecha de expiración y badge con días restantes o indicador de vencida.
  - [x] Los conductores con licencias vigentes por más de 30 días no saturan este widget.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] Si todos los conductores están al día, el widget debe mostrar el estado vacío informativo: *"Todas las licencias al día"*.
...
- **Resultado Esperado (Happy Path):**
  - [x] Se carga la página interactiva con la documentación operativa de CarFleet: Flujo de vida de viajes, diagrama de estados, políticas de combustible, matriz de roles y checklist de puesta en marcha.
...
- **Resultado Esperado (Happy Path):**
  - [x] La tabla lista el viaje cerrado `TRIP-2026-0001` mostrando Solicitante, Conductor, Odómetros (12.000 km $\rightarrow$ 12.250 km), Distancia (250 km), Fecha Real de Salida/Llegada y Estado `Cerrado`.
  - [x] La acción de fila **Ver Detalle del Viaje** permite consultar el expediente completo.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] **Anti-Bypass de Dominio:** La relación de viajes en la ficha del vehículo **es estrictamente de solo lectura (Read-Only)**. No deben existir botones de "Crear Viaje", "Editar Viaje" ni "Eliminar Viaje" dentro de esta tabla para evitar saltarse las validaciones de despacho.
...
- **Resultado Esperado (Happy Path):**
  - [x] **Kilómetros Recorridos:** Muestra la distancia acumulada de los viajes finalizados/cerrados de ese vehículo específico.
  - [x] **Rendimiento de Combustible:** Despliega los `km/galón` específicos del automotor calculados sobre sus abastecimientos.
  - [x] **Galones Consumidos:** Sumatoria de galones registrados en sus vouchers de combustible.
  - [x] **Costo por Kilómetro:** Valor monetario promedio por cada kilómetro recorrido (`$ / km`).
...
- **Resultado Esperado (Happy Path):**
  - [x] Se listan los registros de combustible asociados (ej: `V-BOG-78901` de 14.50 gal y `V-BASE-0091` de 15.00 gal).
  - [x] Se reflejan los costos, odómetros de carga y las fechas de los vouchers.
...
- **Resultado Esperado (Happy Path):**
  - [x] La factura se crea exitosamente en estado `Emitida` (badge amarillo).
  - [x] Se genera automáticamente el consecutivo correlativo único con formato `FACT-YYYY-NNNN` (ej: `FACT-2026-0001`).
  - [x] El valor subtotal del servicio se liquida correctamente:  
        $$\text{Subtotal} = \max(50.000, 250 \text{ km} \times 3.500) = \$ 875.000\text{ COP}$$
  - [x] El monto total de la factura queda guardado en `$ 875.000 COP`.
  - [x] En la lista de facturas (`/admin/invoices`), el registro aparece ordenado con badge de estado, cliente, conteo de servicios (1 servicio) y total formateado.
...
- **Resultado Esperado (Happy Path):**
  - [x] La vista renderiza una factura comercial con diseño corporativo:
  ...
  - [x] El botón superior **🖨️ Imprimir / Guardar como PDF** abre el cuadro de diálogo de impresión nativo del navegador sin desconfigurar los estilos.
  - [x] Al imprimir o exportar a PDF, el botón de impresión se oculta automáticamente (`@media print`).
...
- **Resultado Esperado (Happy Path):**
  - [x] La factura transiciona a estado `Pagada` con badge verde.
  - [x] El sistema emite notificación de éxito: *"La factura FACT-2026-0001 ha sido marcada como pagada"*.
  - [x] Los botones de acción para **Marcar Pagada** y **Anular Factura** desaparecen de la fila.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] **Inmutabilidad de Factura Pagada:** Una factura en estado `Pagada` **nunca debe poder anularse ni modificarse**. Si se intentara forzar la anulación por backend, debe arrojar `InvoiceImmutableException` (no puedo probar tal anulacion forzada por backend, pero tampoco podrian los usuarios normales, no? asi que veo irrelevante tratar de probarlo).
...
- **Resultado Esperado (Happy Path):**
  - [x] La factura `FACT-2026-0002` pasa a estado `Anulada` (badge rojo) y en sus notas queda anexada la traza: *"Motivo anulación: Error en tarifa pactada..."*.
  - [x] **Liberación Inmediata de Viajes:** El viaje `TRIP-2026-0002` vuelve a figurar disponible en el selector de viajes para ser facturado en una nueva cuenta de cobro sin bloqueos.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] **Anulación sin Motivo:** No debe permitir anular una factura si el campo de justificación se deja en blanco.
  - [x] **Viajes Huérfanos/Bloqueados:** Los viajes de una factura anulada no deben quedar bloqueados permanentemente.
...
- **Resultado Esperado (Happy Path / Blindaje UI):**
  - [x] El selector reactivo de Filament **solo lista viajes en estado `Cerrado`** (`eligibleForInvoicing`), excluyendo automáticamente viajes programados, en curso, finalizados sin cerrar o cancelados.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] Si se forzara el envío del ID de un viaje abierto por API o DTO, la acción de dominio `GenerateInvoiceAction` **debe lanzar inmediatamente `TripNotEligibleForInvoicingException`**, abortando la transacción de base de datos (Opciones que me pidan forzar casos via backend no seran probados porque los usuarios finales no tendran tal potestad ni capacidad).
...
- **Resultado Esperado (Happy Path):**
  - [x] El viaje `TRIP-2026-0001` **no aparece en la lista de opciones** porque el scope `uninvoiced()` lo filtra automáticamente.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] Si dos despachadores intentan emitir una factura simultáneamente con el mismo viaje, la base de datos y el bloqueo `lockForUpdate()` deben rechazar la segunda solicitud con `TripAlreadyInvoicedException: El viaje TRIP-XXXX ya se encuentra facturado en la factura FACT-YYYY-NNNN` (solo hay 1 tester asi que confio en que funciona bien la validacion de este caso donde solo dejara modificar al registro aquel que haya iniciado la accion primero).
...
- **Resultado Esperado (Happy Path):**
  - [x] El selector de viajes depende reactivamente del cliente seleccionado en el formulario y únicamente carga viajes asociados al `requester_id` actual.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] Si se intentara enviar un DTO con viajes pertenecientes a dos solicitantes distintos (ej: viaje de Consorcio Vial + viaje de Alimentos del Centro), el dominio **debe arrojar `RequesterMismatchException`** indicando que todos los servicios de una factura deben pertenecer al mismo cliente (En la lista desplegable solamente salen viajes asociados al cliente solicitante).
...
- **Resultado Esperado (Happy Path):**
  - [x] El formulario y campos de observaciones se presentan deshabilitados.
  - [x] Las acciones de cambio de estado están ocultas.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] No debe ser posible re-anular una factura pagada ni marcar como pagada una factura que ya fue anulada.
...
- **Resultado Esperado (Happy Path):**
  - [x] Las tarjetas de rendimiento muestran `0 km`, `0,00 km/gal`, `0,00 gal` y `$ 0 COP` con total estabilidad visual.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] Ninguna consulta matemática debe generar excepciones de división por cero (`DivisionByZeroError`) en PHP.
...
- **Resultado Esperado (Happy Path):**
  - [x] El sistema bloquea el acceso inmediatamente retornando **HTTP 403 Forbidden** o redirigiendo al panel autorizado del conductor.
- **Lo que NUNCA debe suceder (Invariantes / Edge Cases):**
  - [x] Un conductor **nunca debe poder ver tarifas, montos facturados, facturas de clientes ni reportes globales de costos de la flota**.
...
- **Resultado Esperado (Happy Path):**
  - [x] Todos los textos, bordes, badges y contrastes de los gráficos se adaptan correctamente al fondo oscuro sin textos ilegibles, fondos blancos residuales ni problemas de contraste WCAG AA.
...
- **Resultado Esperado (Happy Path):**
  - [x] Los widgets del dashboard se apilan verticalmente en una sola columna limpia.
  - [x] Las tablas de Filament activan desplazamiento horizontal suave o modo tarjeta según corresponda, sin romper el layout ni desbordar la pantalla (se puede hacer desplazamiento horizontal en las cosas que tienen scroll horizontal, pero no hay desplazamiento de tarjetas porque no hay widgets que se superpongan entre si).
